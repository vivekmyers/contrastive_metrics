...
MakeActorFn = Callable[[types.PRNGKey, PolicyNetwork, core.VariableSource], core.Actor]
LoggerLabel = str
LoggerStepsKey = str
...
def get_default_logger_fn(log_to_bigtable: bool = False, log_every: float = 10) -> Callable[[ActorId], loggers.Logger]:
    """Creates an actor logger."""

    def create_logger(actor_id: ActorId):
        return loggers.make_default_logger(
            "actor", save_data=(log_to_bigtable and actor_id == 0), time_delta=log_every, steps_key="actor_steps"
        )
...
def default_evaluator_factory(
    ...
    def evaluator(
        ...
        if logger_fn is not None:
            ...
            logger = loggers.make_default_logger("evaluator", log_to_bigtable, steps_key="actor_steps")

        # Create the run loop and return it.
        return environment_loop.EnvironmentLoop(environment, actor, counter, logger, observers=observers)
...
class DistributedLayout:
    ...
    def __init__(
        ...
        self._multithreading_colocate_learner_and_reverb = multithreading_colocate_learner_and_reverb
    ...
    def counter(self):
        ...
        return savers.CheckpointingRunner(
            counting.Counter(), key="counter", subdirectory="counter", time_delta_minutes=5, **kwargs
        )
    ...
    def learner(
        ...
        if self._builder._config.env_name.startswith(
            "offline"
        ):  # pytype: disable=attribute-error, pylint: disable=protected-access
            adder = self._builder.make_adder(replay)
            env = self._environment_factory(0)
            env_spec = specs.make_environment_spec(env)
            print(env)
            dataset = env.get_dataset()  # pytype: disable=attribute-error
            print(jax.tree_util.tree_map(lambda x: x.shape, dataset))
            for t in tqdm.trange(dataset["observations"].shape[0]):
                ...
                ts = dm_env.TimeStep(
                    ...
                    observation=np.concatenate(
                        [dataset["observations"][t], dataset["infos/goal"][t]], dtype=env_spec.observations.dtype
                    ),
                )
                if t == 0 or dataset["timeouts"][t - 1]:
                    adder.add_first(ts)  # pytype: disable=attribute-error
                else:
                    adder.add(action=dataset["actions"][t - 1].astype(env_spec.actions.dtype), next_timestep=ts)

                if (
                    self._builder._config.local and t > 10_000
                ):  # pytype: disable=attribute-error, pylint: disable=protected-access
                    break
        ...
        if False and self._prefetch_size > 1:
            ...
            iterator = utils.prefetch(iterator, buffer_size=self._prefetch_size, device=device)
        else:
            logging.info("Not prefetching the iterator.")

        counter = counting.Counter(counter, "learner")

        learner = self._builder.make_learner(random_key, networks, iterator, replay, counter)
        kwargs = {}
        if self._checkpointing_config:
            kwargs = vars(self._checkpointing_config)
        # Return the learning agent.
        return savers.CheckpointingRunner(
            learner, key="learner", subdirectory="learner", time_delta_minutes=5, **kwargs
        )
    ...
    def actor(
        ...
        # Environments normally require uint32 as a seed.
        environment = self._environment_factory(utils.sample_uint32(environment_key))

        networks = self._network_factory(specs.make_environment_spec(environment))
        policy_network = self._policy_network(networks)
        actor = self._builder.make_actor(actor_key, policy_network, adder, variable_source)
        ...
        return environment_loop.EnvironmentLoop(environment, actor, counter, logger, observers=self._observers)
    ...
    def build(self, name="agent", program: Optional[lp.Program] = None):
        ...
        with program.group("counter"):
            ...
            if self._max_number_of_steps is not None:
                _ = program.add_node(lp.CourierNode(self.coordinator, counter, self._max_number_of_steps))
        ...
        def make_actor(
            random_key: networks_lib.PRNGKey, policy_network: PolicyNetwork, variable_source: core.VariableSource
        ) -> core.Actor:
            return self._builder.make_actor(random_key, policy_network, variable_source=variable_source)

        with program.group("evaluator"):
            for evaluator in self._evaluator_factories:
                evaluator_key, key = jax.random.split(key)
                program.add_node(lp.CourierNode(evaluator, evaluator_key, learner, counter, make_actor))

        with program.group("actor"):
            for actor_id in range(self._num_actors):
                actor_key, key = jax.random.split(key)
                program.add_node(lp.CourierNode(self.actor, actor_key, replay, learner, counter, actor_id))
